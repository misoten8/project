...
namespace WiimoteApi
{
    ...
    public class AccelData : WiimoteData
    {
		...
		public Vector3 GetAccelVectorZero()
		{
			float accel_x;
			float accel_y;
			float accel_z;

			float[] accel = GetAccelZeroPoints();
			accel_x = accel[0];
			accel_y = -accel[2];
			accel_z = -accel[1];
		...
		public bool GetSwing()
		{
			...
			//Debug.Log("加速度:" + ac);
			//　振った判定
			if (ac < 1.45f && ac > 1.0f && _accelOld - accel.z  > 0)
			{
				Debug.Log("振った");
				Debug.Log("ac-accelOld:" + (ac - _accelOld) * 100);
				// ↓使えない
